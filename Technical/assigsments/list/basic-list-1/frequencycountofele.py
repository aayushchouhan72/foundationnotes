...
if not nums:
    ...
    for i in nums:
        if i > 0:
            if i not in vaildlist:
                vaildlist.append(i)
                count.append(nums.count(i))
        else:
            invlaidlist.append(i)

    if not vaildlist:
        print("Not Valid number in list..")
    else:
        maxcount = max(count)
        mincount = min(count)
        ...
        for n, m in zip(vaildlist, count):
            print(f"{n} ---> {m}")
        ...
        print(f"Most Frequent = {maxfreq}")
        print(f"Min Frequent = {minfreq}")

        if invlaidlist:
            print("Invalid numbers =", invlaidlist)